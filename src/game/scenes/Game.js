...
Game.Game.prototype = {
  ...
  update: function () {
    ...
    this.physics.arcade.overlap(
      this._explosion,
      this._bombs,
      function (explosion, detected_bomb) {
    ...
    this.physics.arcade.overlap(
      this._bomberman,
      this._specialties.getByName('power'),
      function (hero, power) {
    ...
    this.physics.arcade.overlap(
      this._bomberman,
      this._specialties.getByName('door'),
      function (hero, door) {
        if (
          this._enemies.length == 0 &&
          this._timerGame.running &&
          Math.round(this._bomberman.body.x) == Math.round(door.body.x) &&
          Math.round(this._bomberman.body.y) == Math.round(door.body.y)
        )
          this.win();
      },
      null,
    ...
    this.physics.arcade.overlap(
      this._explosion,
      this._brick,
      function (fragment, brick) {
        brick.animations.play('destroy', 10, false, true);
        if (brick.name == 'door-brick')
          this._specialties.getByName('door').revive();
        else if (brick.name == 'power-brick')
          this._specialties.getByName('power').revive();
        fragment.kill();
      },
      null,
      this
  ...
  createMap: function () {
    ...
    if (typeof this._stageBomberman.map != 'undefined')
      this._map.objects.Objects = this._stageBomberman.map;
    ...
    this._brick.forEach(function (element) {
      ...
      this._brickPosition.push(
        Math.round(element.body.x) + ',' + Math.round(element.body.y)
      );
    }, this);
  ...
  updateCharacter: function (power) {
    ...
    if (power == 'ExtendExplosion') {
      ...
      this._labels
        .getByName('SCORE')
        .setText(this._stageBomberman.stage_points);
  ...
  createStatistics: function () {
    ...
    timerGame.loop(
      ...
      function () {
        ...
        if (this._stageBomberman.stage_time < 0) {
          ...
          this._labels
            .getByName('TIME')
            .setText('TIME: ' + this._stageBomberman.stage_time);
      },
      this
    );
    ...
    information = this.add.text(
      distance,
      22,
      this._stageBomberman.points,
      style
    );
    information.fixedToCamera = true;
  ...
  exploitBomb: function (bomb) {
    if (bomb && !this._timers[3].running) {
      ...
      if (this._bombs.countLiving() > 0) this._timers[3].start();
  ...
  putEnemy: function (type, replace, x, y) {
    var coords,
      position =
        this._map.objects.Crossroads[
          Math.floor(Math.random() * this._map.objects.Crossroads.length + 0)
        ];
    ...
    if (index != -1) {
      ...
      if (
        replace ||
        this.physics.arcade.overlap(enemy, this._bomberman, null, null, this)
      ) {
    ...
    this._enemies.forEach(function (enemy) {
      if (enemy.autonomy) {
        if (enemy.name == 'ballon' || enemy.name == 'snow') {
          var move = this.physics.arcade.overlap(
            enemy,
            this._crossroads,
            null,
            function (x, y) {
          ...
          if (move || enemy.body.speed == 0) {
            ...
            this._movements[
              this.rnd.integerInRange(0, this._movements.length - 1)
            ](enemy);
          }
        } else if (enemy.name == 'coin') {
          if (this._bomberman.exists) {
            ...
            if (
              Math.round(this._bomberman.body.x) > Math.round(enemy.body.x) &&
              Math.round(this._bomberman.body.y) != Math.round(enemy.body.y)
            ) {
